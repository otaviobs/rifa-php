...
(function($){
	...
	del = function(_,e,id,pg,tipoFoto,pastaFoto){
		var l = _.ld,ok;
		if(l[id]||!confirm('Você deseja banir '+_.mt+'?'))return;
		e = $(e).addClass('load');
		l[id] = 1;
		$.post('ajax/delete.json',{id:id,pg:pg,tipoFoto:tipoFoto,pastaFoto:pastaFoto},function(x){//NOVO SÓ PÁGINA DELETE
			//if(x.m)alert(x.m);
			if(x.m){
				$('#sys-notification').show();
				var answer = x.ok==1?'success':'danger';
	...
	copy = function(_,e,id,b){
		...
		l[id] = 1;
		$.post('ajax/copy/'+_.a+'.json',{id:id},function(x){
			if(x.m){
				$('#sys-notification').show();
				var answer = x.ok==1?'success':'danger';
				var icon = x.ok==1?'check':'question';
	...
	otimizar = function(_,e,id,b){
		...
		e = $(e);
		s = e.hasClass(i[1])?1:0;
		if(l[id]||!confirm('Você deseja otimizar '+_.mt+'?'))return;
		l[id] = 1;
		$.post('ajax/otimizar.json',{id:id},function(x){
			if(x.m){
				$('#sys-notification').show();
				var answer = x.ok==1?'success':'danger';
				var icon = x.ok==1?'check':'question';
...
//	ob:order-bottom,
				...
				if($.isArray(v.bt))$.each(v.bt,function(j,w){
					...
					if($.type(w)==='object'){
						...
						if(w.tt===2)_w = {t:'Banir',i:'idel',ok:null,f:function(e,c,r){del(c,e,r.id,r.pg,r.tipoFoto,r.pastaFoto);}};
						...
						if(w.tt===6)_w = {m:'s',ld:null,u:null,a:null,i:['otimizar'],t:['Otimizar'],ok:null,f:function(e,c,r,t,b){otimizar(c,e,r.id,b,r.pg);}};
	...
	busca = function(_,p){
		...
		$.post('ajax/get/'+_.c.a+'.json',$.param(d),function(x){
			//if(x.m)alert(x.m);
			if(x.m){
				$('#sys-notification').show();
				var answer = x.ok==1?'success':'danger';
		...
		if($.isArray(_.rs))$.each(_.rs,function(i,v){
			...
			$.each(_.th,function(j,w){
				...
				if(w.t===0){
					...
					var a = w.a||'',b = w.b||'',C = v[w.c]||'',C2 = v[w.c2]||'',img = a+C+b,img2 = C2,il = w.il,auto = w.auto,bg = w.bg;
					cor = C?img:w.d?a+w.d+b:'';
					cor2 = C2?img2:'';
					if(!C)il = w.dil,auto = w.dauto,bg = w.dbg;
					if(cor2)cor = $('<div style="width:103px;height:84px;background:repeating-linear-gradient(0deg, '+cor+', '+cor+' 10px, '+cor2+' 10px, '+cor2+' 19px)"/>');
					else cor = $('<div style="width:103px;height:84px;background:'+cor+'"/>');
					if(w.l===1)c.addClass('iline');
					else if(w.l===2){
						c.addClass('box-img');
						img = $('<span><span></span></span>').addClass('box-img-center').append(img);
