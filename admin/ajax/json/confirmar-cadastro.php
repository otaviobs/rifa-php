...
if(!$s->idu){
	...
	elseif($p1!=$p2&&$x->c='p2')$x->m = 'Confirmação da nova senha diferente!';
		...
		$p1 = sha1($p1);
		$x->c = 'id';
		if($rs=$b->query("select email,codigo from Usuarios where id_usuario=$id limit 1")->fetchObject()){
			$x->c = 'email';
			if(strtoupper($rs->email)==strtoupper($email)){
				$x->c = 'cod';
				if($rs->codigo==$codigo){
					$x->c = '';
					if($b->exec("update Usuarios set senha='$p1',codigo=null where id_usuario=$id limit 1")){
						$x->m = 'Cadastro confirmado com sucesso!';
						$x->ok = 1;
					}else $x->m = 'Erro ao confirmar cadastro!';
				}elseif($rs->codigo)$x->m = 'Código Diferente!';
