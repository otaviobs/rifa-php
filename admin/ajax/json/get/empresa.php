...
if($s->adm()){
	...
	if($q)$w[] = "(e.nome like '$qel' or e.email like '$qel' or e.area like '$qel'".($_rel?" or (match(e.nome,e.email,e.area)against('$q' in boolean mode))":'').' )';
	$w = count($w)?'where '.implode(' and ',$w):'';
	$pgn = pgnl($x->sql_count="$sql $w",$_qt,$_pg);
	if($pgn->e)$pg = $pgn->c;

	/*$sql .= ' left join cat c on e.idc=e.id';
	$sql .= ' left join scat s on e.ids=s.id';
	$sql .= ' left join fabricante f on e.idf=f.id';*/

	$sql = 'select e.*'.
	($q&&$_rel?",(match(e.n)against('$q' in boolean mode)) rel":'').
	" from $sql $w".
	...
	if($st=$b->query($sql)){
		...
		while($rs=$st->fetchObject()){
			...
			$rs->t .= '';
			$rs->q += 0;
			$rf = $b->query("select * from fotos where idp={$rs->id} and tipo='categoria'")->fetchObject();
			$rpr = $b->query("select * from fotos where idp={$rs->id} and tipo='categoria' and principal and it is not null and iti is not null")->fetchObject();
			...
			$rs->e .= '';
			$rs->pg = $rs->tipoFoto = 'empresa';
			$rs->pastaFoto = 'empresas';
			$x->r[$i++] = $rs;
		}
		$x->i = $i;
