...
if($s->adm()){
	...
	/*if($idc&&$b->query("select * from cat where id=$idc limit 1")->fetchObject())$w[] = "d.idc=$idc";
	if($ids&&$b->query("select * from scat where id=$ids limit 1")->fetchObject())$w[] = "d.ids=$ids";
	if($idf&&$b->query("select * from fabricante where id=$idf limit 1")->fetchObject())$w[] = "d.idf=$idf";*/

	if($q)$w[] = "(d.n like '$qel'".($_rel?" or (match(d.n)against('$q' in boolean mode))":'').' )';
	$w = count($w)?'where '.implode(' and ',$w):'';
	$pgn = pgnl($x->sql_count="$sql $w",$_qt,$_pg);
	if($pgn->e)$pg = $pgn->c;

	/*$sql .= ' left join cat c on d.idc=c.id';
	$sql .= ' left join scat s on d.ids=d.id';
	$sql .= ' left join fabricante f on d.idf=f.id';*/

	$sql = 'select d.*'.
	($q&&$_rel?",(match(d.n)against('$q' in boolean mode)) rel":'').
	" from $sql $w".
	...
	if($st=$b->query($sql)){
		...
		while($rs=$st->fetchObject()){
			...
			//if($rs->tipo=='video')$rs->i = 'http://img.youtube.com/vi/'.$rs->y.'/2.jpg';
 			$rs->cp = $rs->it?1:0;
			$rs->e .= '';
			$rs->pg = $rs->tipoFoto = 'destaque';
			$rs->pastaFoto = 'destaques';
			$x->r[$i++] = $rs;
		}
		$x->i = $i;
