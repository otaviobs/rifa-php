...
if($s->adm()){
	...
	if($q)$w[] = "(c.r like '$qel' or c.nf like '$qel' or c.email like '$qel' or c.cnpj like '$qel'".($_rel?" or (match(c.n)against('$q' in boolean mode))":'').' )';
	$w = count($w)?'where '.implode(' and ',$w):'';
	$pgn = pgnl($x->sql_count="$sql $w",$_qt,$_pg);
	if($pgn->e)$pg = $pgn->c;

	/*$sql .= ' left join cat c on c.idc=c.id';
	$sql .= ' left join scat s on c.ids=c.id';
	...
	if($st=$b->query($sql)){
		...
		while($rs=$st->fetchObject()){
			...
			$rs->ct .= '';
			$rs->q += 0;
			$img = 'upload/concessionarias/'.$rs->i;
			...
			//if($rs->tipo=='video')$rs->i = 'http://img.youtube.com/vi/'.$rs->y.'/2.jpg';
 			$rs->cp = $rs->it?1:0;
			$rs->e .= '';
			$rs->pg = $rs->tipoFoto = 'concessionaria';
			$rs->pastaFoto = 'concessionarias';
			$x->r[$i++] = $rs;
		}
		$x->i = $i;
