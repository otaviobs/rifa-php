...
if($s->adm()){
	...
	if($q)$w[] = "(p.nome like '$qel' or p.sku like '$qel' or p.quantidade_pontos='$qo'".($_rel?" or (match(p.nome,p.sku)against('$q' in boolean mode))":'').' )';
	...
	$sql_total = 'select p.*'.($q&&$_rel?",(match(p.nome,p.sku)against('$q' in boolean mode)) rel":'')." from $sql $w".($_ord?" order by $_ord":'');
	$x->nrows = $b->query($sql_total)->rowCount();

	$sql = 'select p.*,p.id_produto as id'.
	($q&&$_rel?",(match(p.nome,p.sku)against('$q' in boolean mode)) rel":'').
	" from $sql $w".
