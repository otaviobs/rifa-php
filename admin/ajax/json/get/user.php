...
if($s->adm()||$s->ven()||$s->ger()){
	...
	$sql = 'Usuarios u';
	// $w = $s->tipoAdm==1?array("u.id_usuario!=1"):array("u.id_usuario!=1 and u.id_perfil!=1");
	$w = array("u.banido=0 and (u.id_perfil=2 or u.id_perfil=3)");
	...
	if($st=$b->query($x->sql=$sql)){
		...
		while($rs=$st->fetchObject()){
			$rs->id += 0;
			$rs->id_perfil += 0;
			$rs->perfil = $rs->id_perfil==2?'GESTOR':'AUDITOR';
			$rs->s += $rs->status;
			$rs->t += 0;
			$rs->_t = $tip[$rs->t];
			$rs->qt += 0;
