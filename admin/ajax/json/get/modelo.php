...
if($s->adm()){
	...
	if($idf&&$b->query("select * from modelo where id=$idf limit 1")->fetchObject())$w[] = "m.idf=$idf";*/
	...
	$sql .= ' left join modelo f on m.idf=m.id';*/
	...
	if($st=$b->query($sql)){
		...
		while($rs=$st->fetchObject()){
			...
			$rs->t .= '';
			$rs->q += 0;
			$rf = $b->query("select * from fotos where idp={$rs->id} and tipo='modelo'")->fetchObject();
			$rpr = $b->query("select * from fotos where idp={$rs->id} and tipo='modelo' and principal and it is not null and iti is not null")->fetchObject();
			if($rpr->it)$img = 'thumb/'.($rpr->it?$rpr->it:($rf->it?$rf->it:'default.jpg'));
			else $img = $rf->i;
			$rs->i = $img&&file_exists("upload/modelos/$img")?$img:'';
 			$rs->cp = ($rs->i?($rs->it?1:0):1)&&($rs->ih?($rs->ith?1:0):1)?1:0;
			$rs->e .= '';
			$rs->pg = $rs->tipoFoto = 'modelo';
			$rs->pastaFoto = 'modelos';
			$x->r[$i++] = $rs;
		}
		$x->i = $i;
