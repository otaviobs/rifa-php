...
if($s->adm()){
	...
	if($q)$w[] = "(f.n like '$qel' or f.h1 like '$qel'".($_rel?" or (match(f.n)against('$q' in boolean mode))":'').' )';
	$w = count($w)?'where '.implode(' and ',$w):'';
	$pgn = pgnl($x->sql_count="$sql $w",$_qt,$_pg);
	if($pgn->e)$pg = $pgn->c;

	/*$sql .= ' left join cat c on f.idc=f.id';
	$sql .= ' left join scat s on f.ids=s.id';
	$sql .= ' left join fabricante f on f.idf=f.id';*/

	$sql = 'select f.*'.
	($q&&$_rel?",(match(f.n)against('$q' in boolean mode)) rel":'').
	" from $sql $w".
	...
	if($st=$b->query($sql)){
		...
		while($rs=$st->fetchObject()){
			...
			$rs->t .= '';
			$rs->q += 0;
			$rf = $b->query("select * from fotos where idp={$rs->id} and tipo='fabricante'")->fetchObject();
			$rpr = $b->query("select * from fotos where idp={$rs->id} and tipo='fabricante' and principal and it is not null and iti is not null")->fetchObject();
			if($rpr->it)$img = 'thumb/'.($rpr->it?$rpr->it:($rf->it?$rf->it:'default.jpg'));
			else $img = $rf->i;
			$rs->i = $img&&file_exists("upload/fabricantes/$img")?$img:'';
 			$rs->cp = ($rs->i?($rs->it?1:0):1)&&($rs->ih?($rs->ith?1:0):1)?1:0;
			$rs->e .= '';
			$rs->pg = $rs->tipoFoto = 'fabricante';
			$rs->pastaFoto = 'fabricantes';
			$x->r[$i++] = $rs;
		}
		$x->i = $i;
