...
if(true){
	...
	elseif(!$r->uf)$x->m = 'Selecione o estado!';
		...
		if($s->idc){
			if($__cepN=preg_match($cepRE,$r->cep,$_cep))$x->anume = $cepNumber = "{$_cep[1]}{$_cep[2]}";
			$cepRegiao = $b->query("select * from faixaCep WHERE $cepNumber between inicio and fim");
			if($cepRegiao->rowCount())$valorTransportadora = $s->calculaDiaEntregue($s->idc);
		}
		if($x->frete->ok||$cepRegiao->rowCount()){
			$x->pac = $x->frete->pac;
			$x->sedex = $x->frete->sedex;
			$x->esedex = $x->frete->esedex;
			$x->transportadora = $cepRegiao->rowCount()&&$valorTransportadora?$valorTransportadora:0;

			$x->valPac = nreal($x->pac);
			$x->valEsedex = nreal($x->esedex);
			$x->valSedex = nreal($x->sedex);
			$x->valTransportadora = nreal($x->transportadora);
			if($tipo==1)$x->frete->val = $x->pac;
			elseif($tipo==2)$x->frete->val = $x->sedex;
			elseif($tipo==3)$x->frete->val = $x->esedex;
			elseif($tipo==4)$x->frete->val = $x->transportadora;
			...
			$x->opcoes = '
				...
				'.($gratis->frete&&$rc->t>=$gratis->vf?'<div class="col-sm-12">
					<p>
						<label for="gratis">FRETE GRÁTIS - <span>Até 11 dias úteis</span>
							<input type="radio" id="gratis" name="frete" value="5"'.($tipo==5||!$tipo?' checked disabled':'').'>
						</label>
					</p>
				</div>':'').'
				...
				'.($x->transportadora?'<div class="col-sm-12">
					<p>
						<label for="esedex"><img src="assets/img/frete/transportador-xsmall.jpg" class="img-transportadora"/> TRANSPORTADORA<br>'.($x->transportadora>0?'R$ '.$x->valTransportadora:'GRÁTIS').' - <span>Até 5 dias úteis</span>
							<input type="radio" id="transportadora" name="frete" style="height:1.3em;width:1.3em" value="4"'.($tipo==4?' checked disabled':'').'>
						</label>
					</p>
				</div>':'');
