...
if($periodo=='mensal') {
  ...
  $vendedores = $b->query("select * from Usuarios where id_perfil=4 and status=1 and aprovado=1");
  ...
  $detalheVendedores = $b->query("
    SELECT sum(CASE WHEN u.status = 1 THEN 1 ELSE 0 END) as Confirmado,
      sum(CASE WHEN u.status = 1 AND u.aprovado = 1 THEN 1 ELSE 0 END) as Elegivel,
